...
taumT=max(0.064,0.064*(1+1+tanh(speedTaum*(triggerTaum-reserves))))
...
sigmaxnpVar = (1/(1 + exp(sigmaxnSpeed*t-sigmaxnInit)))*(sigmaxnp - sigmaxnp*sigmaxnpNew) +  sigmaxnp*sigmaxnpNew#             #Shock on 
...
unem = 1 - L/pop                     #Unemployment rate (*Real*, *%*)
...
YDh = (1 - tauw)*WL + MIh + ESC + ST - WSC - ilh*Ldh + idep*(1 - mdh)*Dh + DIVfh + DIVbh + Rem*en + GOSh - Insh - Comh + Othhh     #Households income  (*Nominal*, *Flow*)
...
CgTar = fi2*GDP                                                     #Government market consumption (*Nominal*, *Flow*)
...
kappagFR = max(kappagmin,kappag - alphafr*sigmaFR*(FD - FDFR)/(krg*pk) * FRactive)               # kappag fiscal rule

sigmaFR= 1/ (1+exp(-gammafr*((Bg + (Bgfx+debtSwapFXBgFX*(1-decds))*en + (Lgfx+debtSwapFXLgFX*(1-decds))*en + (Lgfxtr+debtSwapFXLgFXtr*(1-decds))*en +Bgtr)/GDP)- dfr))


ikgTar = kappagFR*krg                                                 #Target Government investment demand (*Real*, *Flow*)
IkgTar = ikgTar*pk+scenInv*GDP                                      #Target Government investment demand (*Nominal*, *Flow*)

ST = fi3*Wf*(pop - Lg - Lf - Lb) + fi4 * Wf*pop                     #Social transfers paid to Households (*Nominal*, *Flow*)
STg = fistg*ST                                                      #Social transfers paid by the Government (*Nominal*, *Flow*)

FD = Gt - TR - PSg                                                  #Fiscal deficit of the Government (*Nominal*, *Flow*)
FDFR = fdr*GDP                                                      # fiscal deficit rule   
...
Wfdot = (omegaf0*(adot/a) + omegaf1*(L/pop - omegaf2) + omegaf3*pdot/p)*Wf           #Change in wages paid by NFCs (*Nominal*, **)
...
Wgdot = (omegag0*(adot/a) + omegag1*pdot/p)*Wg               #Change in wages paid by the Government (*Nominal*, *Flow*)
...
Dfxgdot = betaDfxg*(etadfxg*(Lgfx +debtSwapFXLgFX+ Bgfx +debtSwapFXBgFX) - Dfxg)            #Government accumulation of FX deposits (*Nominal*, *Flow*)
...
Bgdot = TFNG - Bgfxdot*en - Lgfxdot*en - Lgfxtrdot*en - debtSwapFXLgFXdot*en - debtSwapFXLgFXtrdot*en - debtSwapFXBgFXdot*en -Bgtrdot - shrDon*iktr*pktr/en       #Government domestic bonds issuances (*Nominal*, *Flow*)
...
pop= 24.404942                         #Labour force (*Real*, **)
...
sigmaxnpNew=1.2                     #Size of the shock on the propensity to export
sigmaxnSpeed=0.48                 #Speed of the shock on the propensity to export
...
alphapop=0.01                     #Labour force growth rate
...
tauf=0.1859935                     #Tax rate on NFCs' profits                    
taub=0.1222547                     #Tax rate on FCs' profits
tauw=0.08986282                    #Tax rate on Households' wage bill

betataum= 15
tauvat= 0.1089564                #Value-added tax rate
...
etag=0.08703155                    #Share of population employed as public employees
...
kappagmin = 0.01                  #minimum government investment rate 
...
fdr=0.03                          #threshold for fiscal deficit - fiscal rule 
dfr = 0.6                         #threshold for public debt - fiscal rule 
FRactive = 0                       #activation
...
fi3= 0.54722189217687        #Share of NFCs' wages paid as unemployment benefit
fi4= 0.34            #0.511678    #0.5936                         #Share of NFCs' wages paid as social transfer
