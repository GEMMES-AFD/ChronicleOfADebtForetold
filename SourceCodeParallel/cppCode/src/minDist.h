...
public:
	// CONSTRUCTOR
	minDist(ODE<T>* aModelIn, const int parmsLengthIn,
		int nYInitParmsIn, int nVarMinDistIn, int* varMinDistIn, T* dataMinDistIn, 
		T* samplingTimeMinDistIn, T* pointsWeightMinDistIn, 
		int* nObsVarMinDistIn) :
		myModel(aModelIn), parmsLength(parmsLengthIn),
		nYInitParms(nYInitParmsIn), nVarMinDist(nVarMinDistIn), varMinDist(varMinDistIn), dataMinDist(dataMinDistIn),
	        samplingTimeMinDist(samplingTimeMinDistIn), pointsWeightMinDist(pointsWeightMinDistIn),
		nObsVarMinDist(nObsVarMinDistIn) {}

	//COPY CONSTRUCTOR (not needed for now, default copy is fine)
/*	minDist(const minDist<T>& other) : 
		myModel(other.myModel), parmsLength(other.parmsLength),
		nVarMinDist(other.nVarMinDist), varMinDist(other.varMinDist),
		dataMinDist(other.dataMinDist),
		samplingTimeMinDist(other.samplingTimeMinDist), pointsWeightMinDist(other.pointsWeightMinDist),
	        nObsVarMinDist(other.nObsVarMinDist) {}
*/
	T Evaluate(T* parms) {
		T out = 0;
		try {
...
protected:
	...
	T getSimulatedValue(const T* trajectory, const int it, const int it1) {
		...
		#else
			throw std::exception("ReturnRK4 must be set to 2 or 3 to compute minDist !");
		#endif
	...
	T EvaluateWithoutTryCatch(T* parms) {
		...
		// COMPUTE TRAJECTORY
		myModel->solve(parms, &parms[nYInitParms], trajectory); // parms includes initial positions
