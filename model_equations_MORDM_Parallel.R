...
kappa0 = ((1/(1 + exp(kappa01*temp-kappa02)))*(kappa03 - kappa03*kappa04) +  kappa03*kappa04)


# lambdaiktr = (1/(1.0 + exp(-lambdatr0*(t - lambdatr1))))*lambdatr2
iktr = K_0*(delta_tr + alpha_tr* max(0.0000001, (temp - 4))^gamma_tr * exp(- beta_tr*(temp - 4)))
# lambdaiktr*K_0 + adj_iktr                    #NDC investment  (*Real*, *Flow*)
Iktr = iktr*pktr                                                                #NDC investment (*Nominal*, *Flow*)
adj_iktr_tar = (1/(1.0 + exp(-lambdatr0_adj*(temp - lambdatr1_adj))))*lambdatr2_adj*max((0.013*GDP-Iktr)/pktr, 0) 
...
sigmapcVar = (1/(1 + exp(sigmamSpeed*temp-sigmamInit)))*(sigmapc - sigmapc*sigmapcNew) +  sigmapc*sigmapcNew                      #Shock on the target propensity to import final consumption goods (*Real*, *%*)
sigmapicVar = (1/(1 + exp(sigmamSpeed*temp-sigmamInit)))*(sigmapic - sigmapic*sigmapicNew) +  sigmapic*sigmapicNew#               #Shock on the target propensity to import intermediate consumption goods (*Real*, *%*)
sigmapkVar = (1/(1 + exp(sigmamSpeed*temp-sigmamInit)))*(sigmapk - sigmapk*sigmapkNew) +  sigmapk*sigmapkNew#                     #Shock on the target propensity to import investment goods (*Real*, *%*)
...
sigmaxnpVar = (1/(1 + exp(sigmaxnSpeed*temp-sigmaxnInit)))*(sigmaxnp - sigmaxnp*sigmaxnpNew) +  sigmaxnp*sigmaxnpNew#             #Shock on the target propensity to export non-oil and coal goods (*Real*, *%*)
...
lambdaicb = ((1/(1 + exp(lambdaicb1*temp-lambdaicb2)))*(lambdaicb3 - lambdaicb3*lambdaicb4) +  lambdaicb3*lambdaicb4)
...
premfx = (zetafx0 + zetafx1*(rsk)^zetafx2)*(1+reaction*dsactive*zetafx3*(temp>=4)*exp(-sqrt(rho)*(temp - 0.5*(4+2/rho))^2))                  #Premium on FX loans with the Rest of the World (*Nominal*, *%*) 
...
lambdaicg = ((1/(1 + exp(lambdaicg1*temp-lambdaicg2)))*(lambdaicg3 - lambdaicg3*lambdaicg4) +  lambdaicg3*lambdaicg4)
...
varsigmafdi = (1/(1 + exp(varsigmafdi1*temp-varsigmafdi2)))*(varsigmafdi3 - varsigmafdi3*varsigmafdi4) +  varsigmafdi3*varsigmafdi4   #Shock on total FDI (*Nominal*, *%*) 
...
ratFFX = 1/(1.0 + exp(-betariskFFX*(rsk - MPFFX)))*(UBFFX*(1+dsactive*gammariskFFX*(t>4)*exp(rho*(4-t))) - LBFFX) + LBFFX       #Credit rationing on NFCs' desired FX loans demand with the Rest of the World (*Nominal*, *%*)     #Credit rationing on NFCs' desired FX loans demand with the Rest of the World (*Nominal*, *%*)    #Credit rationing on NFCs' desired FX loans demand with the Rest of the World (*Nominal*, *%*)     #Credit rationing on NFCs' desired FX loans demand with the Rest of the World (*Nominal*, *%*)
...
ratBFX = 1/(1.0 + exp(-betariskBFX*(rsk - MPBFX)))*(UBBFX*(1+dsactive*gammariskBFX*(t>4)*exp(rho*(4-t))) - LBBFX) + LBBFX       #Credit rationing on FCs' desired FX loans demand with the Rest of the World (*Nominal*, *%*)       #Credit rationing on FCs' desired FX loans demand with the Rest of the World (*Nominal*, *%*)
